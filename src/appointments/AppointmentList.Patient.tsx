...
import { getAppointmentsByPatient } from "queries";
...
export default memo(function AppointmentListPatient({
  ...
  const [res, loading, fetchAppointments] = useLazyAppSyncQuery({
    client,
    document: getAppointmentsByPatient,
  ...
  useEffect(() => {
    if (
      res &&
      res.getAppointmentsByPatient &&
      res.getAppointmentsByPatient.items.length
    )
      setAppointments(res.getAppointmentsByPatient.items);
  }, [res]);
  ...
  if (
    ...
    res &&
    res.getAppointmentsByPatient &&
    !res.getAppointmentsByPatient.items.length || !res
  ) {
      ...
      <List
        ...
        onEndReached={() => {
          if (
            res &&
            res.getAppointmentsByPatient &&
            res.getAppointmentsByPatient.nextToken
          ) {
            fetchAppointments({
              patient: currentAuthenticatedUser.id,
              nextToken: res.getAppointmentsByPatient.nextToken
            });
          }
        }}
        ListFooterComponent={() => {
