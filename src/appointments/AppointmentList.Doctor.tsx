...
import { getAppointmentsByDoctor } from "queries";
...
export default memo(function AppointmentList({ navigation: { navigate } }) {
  ...
  async function getAppointments(timestamp: number, refresh: boolean = false) {
    const res = await appsyncFetch({
      client,
      document: getAppointmentsByDoctor,
    ...
    if (
      res &&
      res.getAppointmentsByDoctor &&
      res.getAppointmentsByDoctor.items
    ) {
      return res.getAppointmentsByDoctor.items;
    } else {
      return [];
    }
  }
