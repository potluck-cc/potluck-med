...
export default memo(function ConversationsList({
  navigation: { navigate }
}: Props) {
  const {
    isUserADoctor,
  ...
  useEffect(() => {
    if (isUserADoctor) {
      setConversations(
        res &&
          res.getConversationsByDoctor &&
          res.getConversationsByDoctor.items
          ? res.getConversationsByDoctor.items
          : []
      );
    } else {
      setConversations(
        res &&
          res.getConversationsByPatient &&
          res.getConversationsByPatient.items
          ? res.getConversationsByPatient.items
          : []
      );
    }
  ...
  async function getMoreConversations() {
    if (isUserADoctor) {
      if (
        res &&
        res.getConversationsByDoctor &&
        res.getConversationsByDoctor.nextToken
      ) {
        fetchConversations({
          ...getUserVariables(),
          nextToken: res.getConversationsByDoctor.nextToken
        });
      }
    } else {
      if (
        res &&
        res.getConversationsByPatient &&
        res.getConversationsByPatient.nextToken
      ) {
        fetchConversations({
          ...getUserVariables(),
          nextToken: res.getConversationsByPatient.nextToken
        });
      }
    }
  }
      ...
      <ListItem
        onPress={() => {
          ...
          navigate("Chat", {
            ...
            medToken: isUserADoctor
              ? item.patient.medToken
              : item.doctor.medToken
          });
        }}
  ...
  if (
    (!conversations.length &&
      ...
      res &&
      res.getConversationsByPatient &&
      !res.getConversationsByPatient.items.length) ||
    (!conversations.length &&
      !loading &&
      res &&
      res.getConversationsByDoctor &&
      !res.getConversationsByDoctor.items.length)
  ) {
